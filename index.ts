...
const roomBuilder = async (HBInit: Headless, args: RoomConfigObject) => {
  ...
  room.onPlayerJoin = async (p) => {
    ...
    players.push(new PlayerAugmented(p));
    // Initialize stats for new player
    stats.initPlayerStats(toAug(p));
    welcomePlayer(room, p);
    
    // Auto balance teams for new players
    if (!duringDraft) { // Don't auto-balance during draft
      ...
      if (redCount < 6 || blueCount < 6) { // At least one team has space
        if (redCount < blueCount) {
          room.setPlayerTeam(p.id, 1); // Join red team
          sendMessage(`🔴 Auto-joined Red Team`, p);
        } else if (blueCount < redCount) {
          room.setPlayerTeam(p.id, 2); // Join blue team
          sendMessage(`🔵 Auto-joined Blue Team`, p);
